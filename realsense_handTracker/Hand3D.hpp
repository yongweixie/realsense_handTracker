...
public:
	bool Init() {
		...
		cout << device->SetDepthConfidenceThreshold(15);

		cout << device->SetIVCAMMotionRangeTradeOff(15);

		cout << device->SetIVCAMFilterOption(5);
	...
	void draw(PXCHandData::IHand *ihand) {
		...
		auto isinrange = y > space&&y < yy&&x > space&&x < xx;
		auto iscalib = ihand->IsCalibrated();
		if (iscalib&&isinrange) {
			// 指示器
	...
	cv::Point QueryMassCenterImage(PXCHandData::IHand *ihand) {
		if (ihand == NULL)return cv::Point();
		auto CenterPos = ihand->QueryMassCenterImage();
		return cv::Point(CenterPos.x, CenterPos.y);
	}
...
private:
	...
	int hist_w = 20;
	int space = 20;
	int yy = 480 - space * hist_w;
	int xx = 640 - space * hist_w;
	cv::Mat indicator;
